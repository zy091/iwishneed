...
export default function RequirementImport() {
  ...
  // Get department info from URL params or path
  const departmentParam = searchParams.get('department')
  const isDepartmentView = location.pathname.startsWith('/departments/')
  
  const department = departmentParam === 'tech' ? 'tech'
    : departmentParam === 'creative' ? 'creative'
    : isDepartmentView && location.pathname.includes('/tech') ? 'tech'
    : isDepartmentView && location.pathname.includes('/creative') ? 'creative'
    : 'unknown'
    
  const type: 'tech' | 'creative' | undefined = department === 'tech' ? 'tech' 
    : department === 'creative' ? 'creative' 
    : undefined
    
  const departmentLabel = department === 'tech' ? '技术部' 
    : department === 'creative' ? '创意部' 
    : '通用'
      ...
      {rows.length > 0 && (
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>以原始CSV列展示，用于核对解析是否正确。</CardDescription>
          </CardHeader>
          <CardContent>
