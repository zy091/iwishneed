...
import { FixedAuthService, UserPermissions } from '@/services/fixed-auth-service'
...
interface FixedCommentsSystemProps {
  requirementId: string
  className?: string
}

export default function FixedCommentsSystem({ 
  requirementId, 
  className = '' 
  ...
  const initializeComponent = async () => {
    try {
      ...
      console.log('开始初始化评论组件...')
      
      // 获取用户权限
      const permissions = await FixedAuthService.getCurrentUserPermissions()
  ...
  const fetchComments = async (): Promise<Comment[]> => {
    try {
      console.log('获取评论数据，需求ID:', requirementId)
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          user_external_id: userPermissions.userId,
